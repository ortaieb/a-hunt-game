...
// Create new user
router.post('/', authenticateToken, requireRole('game.admin'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Delete user
router.delete('/:username', authenticateToken, requireRole('game.admin'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Update user
router.put('/:username', authenticateToken, requireRole('game.admin'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!username || !nickname || !roles) {
      res.status(403).json({ error: 'Missing required fields: username, nickname, roles' });
      return;
    }
